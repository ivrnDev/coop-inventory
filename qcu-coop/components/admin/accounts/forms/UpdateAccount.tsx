...
import { createAdmin, getAdminByID, updateAdmin } from "@/lib/api/admin";
...
const UpdateAccount = ({ admin_id }: Props) => {
  ...
  const onSubmit = async (data: ValidateAccount) => {
    if (isAllowed) {
      ...
      try {
        const response = await updateAdmin(form, admin_id);
        if (response.status === 201) {
          await createActivity(
            {
              action: "updated",
              target: "account",
              object: String(admin_id),
            },
            adminId
          );
          toast({
            description: `You have successfully updated account #${admin_id}`,
          });
          router.refresh();
        } else {
          toast({
            variant: "destructive",
            title: `Failed to update account #${admin_id}`,
    ...
    <>
          ...
          <div id="image-container" className="flex space-x-2">
            ...
            <div className="flex">
              ...
              >
                ...
                <p>{imageNumber === 0 ? "Add Image" : "Edit Image"}</p>
                <p>{imageNumber}/1</p>
              </Label>
              <Controller
      ...
      <DialogFooter>
        <Dialog>
          <DialogTrigger
            ref={buttonRef}
            className="bg-green-700 p-3 text-white font-semibold"
          >
